...
def test_dsn(monkeypatch):
    ...
    result = runner.invoke(
        mycli.main.cli, args=["mysql://dsn_user:dsn_passwd@dsn_host:1/dsn_database"]
    )
    ...
    result = runner.invoke(
        mycli.main.cli, args=["mysql://dsn_user@dsn_host:6/dsn_database"]
    )
    assert result.exit_code == 0, result.output + " " + str(result.exception)
    assert (
        MockMyCli.connect_args["user"] == "dsn_user"
...
def test_ssh_config(monkeypatch):
    ...
    with NamedTemporaryFile(mode="w") as ssh_config:
        ...
        result = runner.invoke(
            mycli.main.cli,
            args=["--ssh-config-path", ssh_config.name, "--ssh-config-host", "test"],
        )
        assert result.exit_code == 0, result.output + " " + str(result.exception)
        assert (
            ...
            and MockMyCli.connect_args["ssh_key_filename"]
            == os.getenv("HOME") + "/.ssh/gateway"
        )
