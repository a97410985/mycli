...
class MyCli(object):
    ...
    def connect(
        ...
        if not port:
            ...
            if not host or host == "localhost":
                socket = (
                    cnf["socket"] or cnf["default_socket"] or guess_socket_location()
                )
            ...
            if not WIN and socket:
                ...
                try:
                    ...
                    if [code for code in (2001, 2002, 2003) if code == e.args[0]]:
                        ...
                        self.echo(
                            "Failed to connect to local MySQL server through socket '{}':".format(
                                socket
                            )
                        )
    ...
    def get_password_from_file(self, password_file):
        ...
        if password_file:
            if (
                os.path.isfile(password_file)
                or stat.S_ISFIFO(os.stat(password_file).st_mode)
...
@click.option(
    "--ssl-cert", help="X509 cert in PEM format.", type=click.Path(exists=True)
)
...
@click.option(
    "--local-infile", type=bool, help="Enable/disable LOAD DATA LOCAL INFILE."
)
@click.option(
    "-g", "--login-path", type=str, help="Read this path from the login file."
)
@click.option("-e", "--execute", type=str, help="Execute command and quit.")
@click.option(
    "--init-command", type=str, help="SQL statement to execute after connecting."
)
...
def cli(
    ...
    if ssh_config_host:
        ...
        ssh_key_filename = (
            ssh_key_filename
            if ssh_key_filename
            else ssh_config.get("identityfile", [None])[0]
        )
    ...
    if sys.stdin.isatty():
        ...
        try:
            ...
            click.secho(
                "You might want to try the official mysql client.", err=True, fg="red"
            )
...
def thanks_picker():
    ...
    lines = (
        resources.read_text(mycli, "AUTHORS") + resources.read_text(mycli, "SPONSORS")
    ).split("\n")
