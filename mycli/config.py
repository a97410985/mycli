...
def read_config_file(f, list_values=True):
    ...
    *list_values* set to `True` is the default behavior of ConfigObj.
    ...
    try:
        config = ConfigObj(
            f, interpolation=False, encoding="utf8", list_values=list_values
        )
...
def encrypt_mylogin_cnf(plaintext: IO[str]):
    ...
    def encode_line(plaintext, real_key, buf_len):
        ...
        encrypted_text = b"".join(
            [aes.encrypt(plaintext[i : i + 16]) for i in range(0, len(plaintext), 16)]
        )
...
def read_and_decrypt_mylogin_cnf(f):
    ...
    while True:
        ...
        plain = _remove_pad(
            b"".join(
                [aes.decrypt(cipher[i : i + 16]) for i in range(0, cipher_len, 16)]
            )
        )
        if plain is False:
            continue
